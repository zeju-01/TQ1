// 修复 stock_in_time 和 stock_in_auto_number 字段问题的迁移脚本
const { open } = require('sqlite');
const sqlite3 = require('sqlite3').verbose();
const path = require('path');

async function fixStockInFields() {
  try {
    console.log('修复 stock_in_time 和 stock_in_auto_number 字段问题...\n');
    ...
    // 1. 检查当前字段状态
    console.log('1. 检查当前字段状态...');
    const tableInfo = await db.all("PRAGMA table_info(inventory)");
    const stockInTimeField = tableInfo.find(field => field.name === 'stock_in_time');
    const stockInAutoNumberField = tableInfo.find(field => field.name === 'stock_in_auto_number');
    
    console.log(`   stock_in_time 字段: ${stockInTimeField.type}, ${stockInTimeField.notnull ? 'NOT NULL' : 'NULL'}, 默认值: ${stockInTimeField.dflt_value || '无'}`);
    console.log(`   stock_in_auto_number 字段: ${stockInAutoNumberField.type}, ${stockInAutoNumberField.notnull ? 'NOT NULL' : 'NULL'}, 默认值: ${stockInAutoNumberField.dflt_value || '无'}`);
    
    // 2. 修复表结构（移除 stock_in_time 的默认值）
    console.log('\n2. 修复表结构...');
    ...
    const newStockInTimeField = newTableInfo.find(field => field.name === 'stock_in_time');
    const newStockInAutoNumberField = newTableInfo.find(field => field.name === 'stock_in_auto_number');
    
    console.log(`   修复后 stock_in_time 字段: ${newStockInTimeField.type}, ${newStockInTimeField.notnull ? 'NOT NULL' : 'NULL'}, 默认值: ${newStockInTimeField.dflt_value || '无'}`);
    console.log(`   修复后 stock_in_auto_number 字段: ${newStockInAutoNumberField.type}, ${newStockInAutoNumberField.notnull ? 'NOT NULL' : 'NULL'}, 默认值: ${newStockInAutoNumberField.dflt_value || '无'}`);
    
    if (!newStockInTimeField.dflt_value) {
      console.log('   ✅ stock_in_time 字段默认值已移除');
    } else {
      console.log('   ❌ stock_in_time 字段默认值移除失败');
    }
    
    await db.close();
    console.log('\n✅ stock_in_time 和 stock_in_auto_number 字段问题修复完成！');
